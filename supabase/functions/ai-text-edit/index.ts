...
serve(async (req) => {
  ...
  try {
    const { text, action, tone, prompt } = await req.json();
    const LOVABLE_API_KEY = Deno.env.get("LOVABLE_API_KEY");
    if (!LOVABLE_API_KEY) throw new Error("LOVABLE_API_KEY is not configured");

    let systemPrompt = "You are a professional text editor for an ebook creator. Return ONLY the edited text with no explanations, no quotes, no markdown formatting. Just the raw edited text.";
