...
export default function BookSettingsPanel({
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-background">
          ...
          {hasChanges && (
            <div className="mb-6 p-4 rounded-xl border border-accent/20 bg-accent/[0.03] flex items-center gap-3">
              ...
              <div className="flex-1 flex flex-wrap gap-x-4 gap-y-1 text-xs text-foreground/80">
                ...
                {bookData.chapterContentType !== initialData.chapterContentType && (
                  <span>Content type changed</span>
                )}
                ...
                {USE_CASES.map(uc => (
                  <button key={uc.id} onClick={() => applyUseCase(uc.id)}
                    ...
                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${
                      selectedUseCase === uc.id ? "bg-accent/10" : "bg-foreground/[0.04]"
                    }`}>
            ...
            <section id="settings-length">
              ...
              <div className="px-1">
                <Slider
                  ...
                  onValueChange={([v]) => onBookDataChange(prev => ({ ...prev, wordsPerChapter: v }))}
                  ...
                  return (
                    <button key={t.id} onClick={() => onBookDataChange(prev => ({ ...prev, tone: t.id }))}
                      className={`flex items-start gap-3 px-4 py-3.5 rounded-xl border-2 text-left transition-all ${
                        isActive ? "border-accent bg-accent/[0.04]" : "border-foreground/[0.08] hover:border-foreground/[0.15]"
                      }`}>
                    ...
                    <button key={m.id} onClick={() => onBookDataChange(prev => ({ ...prev, model: m.id }))}
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        <div className="text-center">
                          <div className="flex gap-0.5">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <Zap key={i} size={8} className={i < m.speed ? "text-amber-500 fill-amber-500" : "text-foreground/10"} />
                            ))}
                          </div>
                        ...
                        <div className="text-center">
                          <div className="flex gap-0.5">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <Star key={i} size={8} className={i < m.quality ? "text-accent fill-accent" : "text-foreground/10"} />
                            ))}
                          </div>
              ...
              <div className="flex items-center justify-between p-5 rounded-xl bg-accent/[0.03] border border-accent/20">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={onClose}
                    className="px-4 py-2.5 rounded-xl border border-foreground/[0.1] text-sm font-medium hover:bg-foreground/[0.04] transition-colors">
                    Cancel
                  </button>
      ...
      {/* ─── RIGHT: Live Preview ─── */}
      <div className="w-64 shrink-0 border-l border-foreground/[0.06] py-8 px-5 hidden xl:block">
        <div className="sticky top-8">
          <h3 className="text-xs font-bold uppercase tracking-wider text-foreground/70 mb-5">Live Summary</h3>
          <div className="space-y-4">
            ...
            <SummaryRow icon={<FileText size={14} />} label="Words/Chapter" value={bookData.wordsPerChapter.toLocaleString()} />
          ...
          <div className="mt-6 pt-5 border-t border-foreground/[0.06]">
            <h4 className="text-[10px] font-bold uppercase tracking-wider text-foreground/50 mb-3">Estimated Output</h4>
            <div className="space-y-2">
          ...
          {/* Project Actions */}
          <div className="mt-6 pt-5 border-t border-foreground/[0.06]">
            <h4 className="text-[10px] font-bold uppercase tracking-wider text-foreground/50 mb-3">Project</h4>
